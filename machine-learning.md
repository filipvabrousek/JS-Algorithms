...
class Circuit {
  ...
  backward(gradient_top) { // takes pull from above
...
class SVM {
  ...
  forward(x, y) { // assume x and y are Units
...
// Sum of synapses, treshold defines if it will trigger
const neuron = ({synapses = [], treshold = 1} = {}) => ({
   synapses,
    treshold
});


// weight from 0 to 1 and curr. value from -1 to +1
...
// takes treshold and synapses, sums the result and decides whether to fire
function shouldTrigger({ treshold, synapses}){
...
const n1 = neuron({synapses: [
    synapse({value: -0.2}),
    synapse({weight: 0, value: 1}),
    synapse({weigth: 0.5, value: 0.8})
],
    treshold: 0.3
});

const n2 = neuron({synapses: [
    synapse({value: -0.2}),
    synapse({weight: 0, value: 1}),
    synapse({weigth: 0.5, value: 0.8})
],
...
const willTriggerN1 = shouldTrigger(n1);
const willTriggerN2 = shouldTrigger(n2);
