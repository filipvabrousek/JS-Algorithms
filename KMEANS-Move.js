...
class Means {
	...
	assign(centrs) {
		...
		points.forEach((val, i) => {
			...
			(sqzerox || sqzeroy) < 110 ? APoints.push(points[i]) : 0;
			(sqonex || sqoney) < 110 ? BPoints.push(points[i]) : 0;
		});
...
getNewMeans(amean, bmean){
    let newmeans = [];
        newmeans.push(amean, bmean);
    return newmeans;
}
	...
	drawMean() {
		let ctx = this.ctx;
		let means = this.initMeans() ; // init random means (stop this from running)
        ...
        /* we should change means to average position of assigned points
        but they are reset at the beginning of each function
        means[0] = amean
        means[1] = bmean, 
        */
